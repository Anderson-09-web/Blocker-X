...
export default function AdminUsersPage() {
  ...
  const handle = (action: string, userId: string) => {
    ...
    fns[action].mutate({ userId }, {
      onSuccess: () => { refresh(); toast({ title: `User ${action}ned` }); },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative">
        ...
        <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by username or Discord ID..." className="pl-10" data-testid="input-search-users" />
      </div>
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="divide-y divide-border/30 overflow-x-auto">
              ...
              {users.map((u: any) => (
                <div key={u.id} className="grid grid-cols-[1fr_auto_auto_auto] gap-4 items-center px-6 py-3" data-testid={`row-user-${u.id}`}>
                  ...
                  <div className="flex items-center gap-1">
                    {u.isBanned ? (
                      <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-green-400 hover:bg-green-500/10" title="Unban" onClick={() => handle("unban", u.id)} data-testid={`button-unban-${u.id}`}>
                        <Shield className="w-3.5 h-3.5" />
                      </Button>
                    ) : (
                      <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-yellow-400 hover:bg-yellow-500/10" title="Ban" onClick={() => handle("ban", u.id)} data-testid={`button-ban-${u.id}`}>
                        <ShieldOff className="w-3.5 h-3.5" />
                      </Button>
                    )}
                    {u.plan === "free" ? (
                      <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-amber-400 hover:bg-amber-500/10" title="Upgrade to Premium" onClick={() => handle("upgrade", u.id)} data-testid={`button-upgrade-${u.id}`}>
                        <ChevronUp className="w-3.5 h-3.5" />
                      </Button>
                    ) : (
                      <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-gray-400 hover:bg-gray-500/10" title="Downgrade to Free" onClick={() => handle("downgrade", u.id)} data-testid={`button-downgrade-${u.id}`}>
                        <ChevronDown className="w-3.5 h-3.5" />
                      </Button>
                    )}
                    <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-destructive hover:bg-destructive/10" title="Delete" onClick={() => handle("delete", u.id)} data-testid={`button-delete-user-${u.id}`}>
